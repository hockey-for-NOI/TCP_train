		...
		*m_pipe = -1;
		...
		try
		{
			...
			if (opc == Protocol::READ_MAGIC)
			{
				...
				unsigned crc32 = 0;
				for (unsigned i=0; i<filelen; i+=BUF_SIZE)
				{
			...
			else
			{
				...
				crc32 ^= 0xffffffff;
				unsigned chk;
				packed_read(pipe, &chk, 4);
				if (chk != crc32)
				{
					fdesc.discard();
					throw std::runtime_error("CRC32 Failed.");
				}
			}
		}
